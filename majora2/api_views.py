...
def get_pag_by_qc(request):
    def f(request, api_o, json_data, user=None):
        ...
        if json_data.get("pass_only"):
            pass_pags = models.PAGQualityReportEquivalenceGroup.objects.filter(test_group=t_group, pag__is_latest=True, is_pass=True, pag__is_suppressed=False).select_related('pag').prefetch_related('pag__tagged_artifacts')
        else:
            pass_pags = models.PAGQualityReportEquivalenceGroup.objects.filter(test_group=t_group, pag__is_latest=True, pag__is_suppressed=False).select_related('pag').prefetch_related('pag__tagged_artifacts')
